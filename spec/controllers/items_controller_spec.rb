	...
	describe 'GET#trade_items' do
		...
		context 'Trading process' do
			it 'When trade Successful' do 
				get :trade_items, params: {"trade_to": survivor1.name, "trade_by": survivor2.name, "trade_to_items": [ {"item": "soup", "quantity": 1} ], "trade_by_items": [ {"item": "AK47", "quantity": 1} ] }
			...
			it 'survivor not found' do 
				...
				expect(JSON.parse(response.body)['errors']).to eq('Both Serrvivors not found')
			end

			it 'when trade quantity not match' do 
				get :trade_items, params: {"trade_to": survivor2.name, "trade_by": survivor1.name, "trade_to_items": [ {"item": "soup", "quantity": 1} ], "trade_by_items": [ {"item": "AK47", "quantity": 1} ] }
